...
function utils_log() {
	if (window.DEBUG) {
		const err = new Error();
		console.log(parseInt(Date.now().toString().substr(4), 10), ...arguments, err.stack.split('\n')[2].replace(/\(.*\)/, ''));
	}
}
...
function Divider(props) {
	return Object(preact_min["h"])('div', { 'class': `divider ${props.vertical ? 'divider--vertical' : ''}` });
}

var common__ref = Object(preact_min["h"])(
	'span',
	{ 'class': 'beta-tag' },
	'Beta'
);
...
const DEFAULT_PROFILE_IMG = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'%3E%3Cpath fill='%23ccc' d='M12,19.2C9.5,19.2 7.29,17.92 6,16C6.03,14 10,12.9 12,12.9C14,12.9 17.97,14 18,16C16.71,17.92 14.5,19.2 12,19.2M12,5A3,3 0 0,1 15,8A3,3 0 0,1 12,11A3,3 0 0,1 9,8A3,3 0 0,1 12,5M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12C22,6.47 17.5,2 12,2Z'/%3E%3C/svg%3E";
		...
		({ i18n }) => Object(preact_min["h"])(
			...
			Object(preact_min["h"])(
				...
				Object(preact_min["h"])(
					...
					{
						id: 'saveBtn',
						'class': `btn btn--dark hint--rounded hint--bottom-left ${props.isSaving ? 'is-loading' : ''} ${props.unsavedEditCount ? 'is-marked' : 0}`,
				...
				Object(preact_min["h"])(
					...
					{
						id: 'openItemsBtn',
						'class': `btn btn--dark hint--rounded hint--bottom-left ${props.isFetchingItems ? 'is-loading' : ''}`,
...
// EXTERNAL MODULE: ../node_modules/@emmetio/codemirror-plugin/dist/emmet-codemirror-plugin.es.js + 24 modules
	...
	async loadDeps() {
		if (this.props.type === 'monaco') {
			if (!monacoDepsDeferred) {
				...
				__webpack_require__.e/* import() */(4).then(__webpack_require__.bind(null, "lbPO")).then(() => {
...
var ContentWrap__ref5 = Object(preact_min["h"])(
	'svg',
	null,
	Object(preact_min["h"])('use', { xlinkHref: '#code-brace-icon' })
);

var ContentWrap__ref6 = Object(preact_min["h"])('span', { 'class': 'caret' });
...
var ContentWrap__ref13 = Object(preact_min["h"])(
	'svg',
	null,
...
var _ref15 = Object(preact_min["h"])('span', { 'class': 'caret' });
...
let ContentWrap_ContentWrap = class ContentWrap extends preact_min["Component"] {
  ...
  */
	setPreviewContent(isForced, isManual) {
		if (!isManual) {
			let autoPreview = window.forcedSettings.autoPreview !== undefined ? window.forcedSettings.autoPreview : this.props.prefs.autoPreview;
			if (!autoPreview) {
				return;
			}
		}
				...
				Object(preact_min["h"])(
					...
					Object(preact_min["h"])(
						...
						Object(preact_min["h"])(
							'div',
							{ 'class': 'code-wrap__header-right-options' },
							this.props.currentItem.htmlMode === HtmlModes.HTML ? Object(preact_min["h"])(
								'a',
								{
									'class': 'code-wrap__header-btn',
									title: 'Format code',
					...
					Object(preact_min["h"])(CodeEditor_CodeEditor, {
						type: window.forcedSettings.isMonacoEditorOn === true || this.props.prefs.isMonacoEditorOn && window.forcedSettings.isMonacoEditorOn !== false ? 'monaco' : 'codemirror',
						options: {
							mode: 'htmlmixed',
							profile: 'xhtml',
...
function File({
	...
	return Object(preact_min["h"])(
		...
		file === fileBeingRenamed ? Object(preact_min["h"])('input', {
			...
			Object(preact_min["h"])(
				'div',
				{ 'class': 'flex flex-v-center ov-h' },
				Object(preact_min["h"])(FileIcon, { file: file }),
				Object(preact_min["h"])(
					'div',
					{ 'class': 'sidebar__file-name' },
					file.name
				)
			),
			Object(preact_min["h"])(
  ...
  */
	...
	render() {
		...
		return Object(preact_min["h"])(
			...
			Object(preact_min["h"])(
				'div',
				{ 'class': 'd-f jc-sb ai-c', style: 'padding: 5px 10px' },
				'Files',
				Object(preact_min["h"])(
					'div',
					{ 'class': 'flex ai-c' },
...
const PREVIEW_FRAME_HOST = window.DEBUG ? 'http://localhost:7888' : `https://preview.${location.host}`;

/* global htmlCodeEl
 */

var ContentWrapFiles__ref = Object(preact_min["h"])(
	'svg',
	null,
	Object(preact_min["h"])('use', { xlinkHref: '#code-brace-icon' })
);

var ContentWrapFiles__ref2 = Object(preact_min["h"])('a', {
	'class': 'js-code-collapse-btn  code-wrap__header-btn  code-wrap__collapse-btn',
	title: 'Toggle code pane'
});
...
let ContentWrapFiles_ContentWrapFiles = class ContentWrapFiles extends preact_min["Component"] {
	...
	componentWillUpdate(nextProps) {
		...
		if (this.state.selectedFile && !getFileFromPath(nextProps.currentItem.files, this.state.selectedFile.path).file) {
			this.state.selectedFile = null;
		}
	...
	createPreviewFile() {
		...
		if (this.detachedWindow) {
			utils_log('✉️ Sending message to detached window');
			this.detachedWindow.postMessage({ url: `${PREVIEW_FRAME_HOST}/index.html` }, '*');
		} else {
			setTimeout(() => {
	...
	render() {
		return Object(preact_min["h"])(
				...
				Object(preact_min["h"])(
					...
					Object(preact_min["h"])(
						...
						Object(preact_min["h"])(
							...
							Object(preact_min["h"])(FileIcon, { file: this.state.selectedFile }),
			...
			Object(preact_min["h"])(
				...
				Object(preact_min["h"])('iframe', { src: `${PREVIEW_FRAME_HOST}/talk.html`, id: 'talkFrame' }),
				Object(preact_min["h"])(PreviewDimension_PreviewDimension, { ref: comp => this.previewDimension = comp }),
				Object(preact_min["h"])(Console_Console, {
					logs: this.state.logs,
					isConsoleOpen: this.state.isConsoleOpen,
...
var Footer__ref18 = Object(preact_min["h"])('div', { 'class': 'footer__separator' });

var Footer__ref19 = Object(preact_min["h"])(
	'svg',
	{ viewBox: '0 0 24 24' },
...
var Footer__ref20 = Object(preact_min["h"])('span', { 'class': 'notifications-btn__dot' });
...
const itemService = {
  ...
  */
	...
	async setItem(id, item) {
		...
		remoteDb.collection('items').doc(id).set(item, {
			merge: true
		}).then(arg => {
  ...
  */
	...
	async unsetItemForUser(itemId) {
		...
		const remoteDb = await window.db.getDb();
		return remoteDb.collection('users').doc(window.user.uid).update({
			[`items.${itemId}`]: index_cjs_default.a.firestore.FieldValue.delete()
		}).then(arg => {
...
var SavedItemPane__ref2 = Object(preact_min["h"])(
	...
	Object(preact_min["h"])('img', { style: 'max-width: 80%; opacity:0.4', src: 'assets/empty.svg' })
);
...
let SavedItemPane_SavedItemPane = class SavedItemPane extends preact_min["Component"] {
	...
	shouldComponentUpdate(nextProps, nextState) {
		return nextProps.items !== this.props.items || nextProps.isOpen !== this.props.isOpen || nextState.filteredItems !== this.state.filteredItems;
	}
	...
	searchInputHandler(e) {
		...
		if (!text) {
			...
			this.setState({
				filteredItems: this.state.items.filter(item => item.title.toLowerCase().indexOf(text) !== -1)
			});
		}
		trackEvent('ui', 'searchInputType');
	}

	render({ isOpen, exportBtnClickHandler }, { filteredItems = this.state.items, items = [] }) {
		return Object(preact_min["h"])(
			'div',
			{
			...
			Object(preact_min["h"])(
				...
				!filteredItems.length && items.length ? SavedItemPane__ref : null,
				filteredItems.map(item => Object(preact_min["h"])(ItemTile, {
					item: item,
				...
				!items.length ? SavedItemPane__ref2 : null
			)
		);
	}
};
...
const jsLibs = [{
	...
	url: 'https://cdn.jsdelivr.net/npm/foundation-sites@6.5.3/dist/js/foundation.min.js',
	label: 'Foundation',
	type: 'js'
}, {
	...
	url: 'https://cdnjs.cloudflare.com/ajax/libs/uikit/3.1.5/js/uikit.min.js',
	label: 'UIkit 3',
	type: 'js'
}, {
...
const cssLibs = [{
	...
	url: 'https://cdn.jsdelivr.net/npm/foundation-sites@6.5.3/dist/css/foundation.min.css',
	label: 'Foundation',
	type: 'css'
}, {
	...
	url: 'https://cdnjs.cloudflare.com/ajax/libs/uikit/3.1.5/css/uikit.min.css',
	label: 'UIkit 3',
	type: 'css'
}, {
...
var Notifications__ref2 = Object(preact_min["h"])(
	...
	' If you like Web Maker, you\'ll probably also like a new product I launched. It\'s the first ever code-golfing game for CSS ->',
	' ',
	Object(preact_min["h"])(
		'a',
		{ href: 'https://cssbattle.dev', target: '_blank' },
		'CSSBattle'
	),
	'.'
);
...
var Notifications__ref4 = Object(preact_min["h"])(
	'a',
	{
...
function Notification(_ref) {
	let { version, isLatest } = _ref,
	    props = Notifications__objectWithoutProperties(_ref, ['version', 'isLatest']);
...
var Notifications__ref13 = Object(preact_min["h"])(
	NotificationItem,
	{ type: 'bug' },
	'JavaScript evaluation failing randomly with errors like const cannot be reassigned.'
);

var Notifications__ref14 = Object(preact_min["h"])(
	NotificationItem,
	{ type: 'bug' },
	'Command palette now opens with ',
	Object(preact_min["h"])(
		'code',
		null,
		'F1'
...
var Notifications__ref16 = Object(preact_min["h"])(
	'p',
	null,
	'Sit tight, because this is going to be some amazing set on updates for you! Read',
	' ',
	Object(preact_min["h"])(
		'a',
		{
...
var Notifications__ref17 = Object(preact_min["h"])(
	...
	Object(preact_min["h"])(
		...
		': First of all, Web Maker has a new home -',
		' ',
		Object(preact_min["h"])(
			'a',
			{ href: 'https://webmaker.app' },
			'https://webmaker.app'
		),
		' \uD83D\uDE00 The old website will continue to run for a month or so, but will eventually start redirecting to the new website. If you have an online account in Web Maker, you need not worry. But if you have some locally saved creations, you should',
		' ',
		Object(preact_min["h"])(
			'a',
			{
	...
	Object(preact_min["h"])(
		...
		': Hear this - you can now work with files just like you do in your local environment! No more working in just 3 panes. Try creating a new creation. "Files Mode" is in beta and you can currently only save 2 creations in this mode.'
	),
	Object(preact_min["h"])(
		'li',
		null,
		...
		': We all love this, don\'t we? Taking frequent actions is now much quicker with the all new Command Palette. Hit Ctrl/Command+Shift+P, and give it a spin.'
	),
	Object(preact_min["h"])(
		'li',
		null,
		...
		': Console inspection becomes more powerful. You can view nested objects, arrays and even DOM Elements!'
	),
	Object(preact_min["h"])(
		'li',
		null,
		...
		': You can choose to use Monaco instead of CodeMirror editor from the settings. Though this is still an experimental feature and might contain bugs.'
	),
	Object(preact_min["h"])(
		'li',
		null,
		...
		': Format your code with everybody\'s favourite',
		' ',
		Object(preact_min["h"])(
			'a',
			{
	...
	Object(preact_min["h"])(
		NotificationItem,
		{ type: 'ui', label: 'Preview dimensions' },
		'You can see the preview frame dimensions when resizing.'
	),
	Object(preact_min["h"])(
		NotificationItem,
		{ type: 'ui', label: 'Settings' },
...
function Notifications(props) {
	return Object(preact_min["h"])(
		...
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '4.0.3' }, props, { isLatest: true }),
			Notifications__ref7
		),
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '4.0.2' }, props),
			Notifications__ref8,
			Notifications__ref9
		),
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '4.0.1' }, props),
		...
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '4.0.0' }, props),
			Notifications__ref16,
			Notifications__ref17
		),
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '3.6.2' }, props),
			Notifications__ref18
		),
		Object(preact_min["h"])(
			Notification,
			Notifications__extends({ version: '3.6.1' }, props),
		...
		Object(preact_min["h"])(
			...
			Object(preact_min["h"])(
				"span",
				{
					...
					style: `visibility:${checked && !showBothLabels ? 'hidden' : 'visible'}`
				},
				labels[0] || 'Off'
			),
			Switch__ref,
			Object(preact_min["h"])(
				"span",
				{
					"aria-hidden": "true",
					"class": "check-switch__status",
					style: `visibility:${checked || showBothLabels ? 'visible' : 'hidden'}`
				},
				labels[1] || 'On'
			)
		),
...
let Tabs_Tabs = class Tabs extends preact_min["Component"] {
	...
	render() {
		...
		return Object(preact_min["h"])(
			'div',
			{ 'class': `tabs ${this.props.horizontal ? 'tabs--horizontal' : ''}` },
			Object(preact_min["h"])(
				'div',
				{
					'class': 'tabs__tablist',
...
var Settings__ref15 = Object(preact_min["h"])(Divider, null);
...
var Settings__ref32 = Object(preact_min["h"])(
	'option',
	{ value: 'typescript' },
	'TypeScript'
);
...
var Settings__ref34 = Object(preact_min["h"])(Divider, null);
...
var Settings__ref40 = Object(preact_min["h"])(
	'option',
	{ value: 'other' },
...
var Settings__ref45 = Object(preact_min["h"])(Divider, null);

var Settings__ref46 = Object(preact_min["h"])(
	HelpText,
	null,
	'Enjoy wonderful particle blasts while you type'
);

var Settings__ref47 = Object(preact_min["h"])(Divider, null);
...
const Profile_DEFAULT_PROFILE_IMG = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'%3E%3Cpath fill='%23ccc' d='M12,19.2C9.5,19.2 7.29,17.92 6,16C6.03,14 10,12.9 12,12.9C14,12.9 17.97,14 18,16C16.71,17.92 14.5,19.2 12,19.2M12,5A3,3 0 0,1 15,8A3,3 0 0,1 12,11A3,3 0 0,1 9,8A3,3 0 0,1 12,5M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12C22,6.47 17.5,2 12,2Z'/%3E%3C/svg%3E";
...
var KeyboardShortcutsModal__ref3 = Object(preact_min["h"])(
	'p',
	null,
	Object(preact_min["h"])(
		'span',
...
var KeyboardShortcutsModal__ref4 = Object(preact_min["h"])(
	'p',
	null,
	Object(preact_min["h"])(
		'span',
...
var KeyboardShortcutsModal__ref5 = Object(preact_min["h"])(
	'p',
	null,
	Object(preact_min["h"])(
		'span',
...
var KeyboardShortcutsModal__ref6 = Object(preact_min["h"])(
	'p',
	null,
	Object(preact_min["h"])(
		'span',
...
var KeyboardShortcutsModal__ref7 = Object(preact_min["h"])(
	'p',
	null,
	Object(preact_min["h"])(
		'span',
...
var KeyboardShortcutsModal__ref8 = Object(preact_min["h"])(
	'div',
	{ 'class': 'ml-2' },
	Object(preact_min["h"])(
		'h2',
	...
	return Object(preact_min["h"])(
		...
		Object(preact_min["h"])(
			...
			Object(preact_min["h"])(
				...
				KeyboardShortcutsModal__ref3,
				KeyboardShortcutsModal__ref4,
				KeyboardShortcutsModal__ref5,
				KeyboardShortcutsModal__ref6,
				KeyboardShortcutsModal__ref7
			),
			KeyboardShortcutsModal__ref8
		)
	);
}
...
var OnboardingModal__ref2 = Object(preact_min["h"])(
	...
	Object(preact_min["h"])(
		...
		Object(preact_min["h"])(
			...
			Object(preact_min["h"])(
				'a',
				{ 'class': 'ml-1 bookmarklet', href: 'https://webmaker.app/app/' },
				Object(preact_min["h"])(
					'svg',
...
var CreateNewModal__ref = Object(preact_min["h"])(
	'g',
	{
		id: 'Page-1',
		stroke: 'none',
	...
	Object(preact_min["h"])(
		'g',
		{ id: 'codepane', fill: '#C5C5C5' },
		Object(preact_min["h"])('rect', { id: 'Rectangle', x: '0', y: '0', width: '89', height: '80', rx: '5' })
	),
	Object(preact_min["h"])(
		'g',
		{
...
var CreateNewModal__ref2 = Object(preact_min["h"])(
	'p',
	{ 'class': 'mb-0' },
	'3 separate panes to write HTML, CSS and JavaScript. Good for quickly testing something.'
);

var CreateNewModal__ref3 = Object(preact_min["h"])(
	'g',
	{
		id: 'Page-1',
		stroke: 'none',
...
var CreateNewModal__ref4 = Object(preact_min["h"])(
	'p',
	{ 'class': 'mb-0' },
	'Manage your code in files. Just like you would have files on your local machine. ',
	Object(preact_min["h"])(BetaTag, null)
);

var CreateNewModal__ref5 = Object(preact_min["h"])(
	'h1',
	{ 'class': 'mt-0' },
	'Create New'
);

var CreateNewModal__ref6 = Object(preact_min["h"])(
	'div',
	{ 'class': 'show-when-extension' },
...
let CreateNewModal_CreateNewModal = class CreateNewModal extends preact_min["Component"] {
	...
	modeChangeHandler(selectedtabIndex) {
		...
		trackEvent('ui', 'newCreationModeChange', !!selectedtabIndex ? 'files' : '3panes');
	}
	render() {
		const {
			show,
		...
		const option1 = Object(preact_min["h"])(
			'div',
			{ style: 'flex:1;', 'class': 'tac' },
			Object(preact_min["h"])(
				'svg',
				{
					width: '25%',
			...
			CreateNewModal__ref2
		);
		const option2 = Object(preact_min["h"])(
			'div',
			{ style: 'flex:1', 'class': 'tac ml-2' },
			Object(preact_min["h"])(
				'svg',
				{
					width: '25%',
				...
				CreateNewModal__ref3
			),
			CreateNewModal__ref4
		);

		return Object(preact_min["h"])(
			Modal_Modal,
			{ show: show, closeHandler: closeHandler },
			CreateNewModal__ref5,
			Object(preact_min["h"])(
				Tabs_Tabs,
				{ horizontal: true, onChange: this.modeChangeHandler },
				Object(preact_min["h"])(
					TabPanel,
					{ label: option1 },
					Object(preact_min["h"])(
						...
						templateList.map(template => {
							return Object(preact_min["h"])(ItemTile, {
								inline: true,
								item: template,
								focusable: true,
...
var Icons__ref19 = Object(preact_min["h"])(
	"symbol",
	{ id: "code-brace-icon", viewBox: "0 0 24 24" },
	Object(preact_min["h"])("path", { d: "M8,3A2,2 0 0,0 6,5V9A2,2 0 0,1 4,11H3V13H4A2,2 0 0,1 6,15V19A2,2 0 0,0 8,21H10V19H8V14A2,2 0 0,0 6,12A2,2 0 0,0 8,10V5H10V3M16,3A2,2 0 0,1 18,5V9A2,2 0 0,0 20,11H21V13H20A2,2 0 0,0 18,15V19A2,2 0 0,1 16,21H14V19H16V14A2,2 0 0,1 18,12A2,2 0 0,1 16,10V5H14V3H16Z" })
);
...
var Icons__ref21 = Object(preact_min["h"])(
	"g",
...
var CommandPalette__ref = Object(preact_min["h"])(
	'p',
	null,
...
let CommandPalette_CommandPalette = class CommandPalette extends preact_min["Component"] {
	...
	keyDownHandler(e) {
		...
		if (e.which === keyboardKeys_ENTER_KEY && this.state.list.length) {
	...
	render() {
		return Object(preact_min["h"])(
			...
			!this.props.files.length && !this.state.list.length ? CommandPalette__ref : null,
			Object(preact_min["h"])(
				'ul',
...
if (location.search) {
	...
	if (match) {
		...
		match.split(',').map(pair => {
			pair = pair.split(':');
			if (pair[1] === 'true') pair[1] = true;else if (pair[1] === 'false') pair[1] = false;
...
let app_App = class App extends preact_min["Component"] {
	...
	async loadLanguage(lang) {
		utils_log('🇯🇲 fetching defninition');

		const catalog = await __webpack_require__("IChQ")(`./${lang}/messages.js`);

		this.setState(state => ({
			catalogs: app__extends({}, state.catalogs, {
	...
	askForUnsavedChanges() {
		return confirm('You have unsaved changes in your current work. Do you want to discard unsaved changes and continue?');
	}
...
<html>
  ...
  */
	...
	componentDidMount() {
		...
		window.addEventListener('keydown', event => {
			...
			if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.keyCode === 53) {
			...
			} else if (event.keyCode === 27 && (event.target.tagName !== 'INPUT' || event.target.id === 'searchInput')) {
				// ESCAPE
				...
				trackEvent('ui', 'openCommandPaletteKeyboardShortcut', !!event.shiftKey ? 'command' : 'files');
				event.preventDefault();
  ...
  */
	updateSetting(settingName, value) {
		...
		let autoPreview = window.forcedSettings.autoPreview !== undefined ? window.forcedSettings : prefs.autoPreview;
  ...
  */
	...
	blankFileTemplateSelectHandler() {
		itemService.getCountOfFileModeItems().then(count => {
			if (count < 2) {
				...
				return alert('"Files mode" is currently in beta and is limited to only 2 creations per user. You have already made 2 creations in Files mode.\n\nNote: You can choose to delete old ones to create new.');
			}
		});
	}
	...
	templateSelectHandler(template, isFileMode) {
		if (isFileMode) {
			itemService.getCountOfFileModeItems().then(count => {
				if (count < 2) {
					fetch(`templates/template-${isFileMode ? 'files-' : ''}${template.id}.json`).then(res => res.json()).then(json => {
						this.forkItem(json);
					});
					this.setState({ isCreateNewModalOpen: false });
				} else {
	...
	prettifyHandler(what) {
		...
		if (typeof what === 'string') {
			prettify({
				...
				this.setState({ currentItem: app__extends({}, this.state.currentItem) }, () => {
	...
	render(props, { catalogs = {}, prefs = {} }) {
		return Object(preact_min["h"])(
			react["I18nProvider"],
			{ language: this.state.prefs.lang, catalogs: catalogs },
			Object(preact_min["h"])(
				'div',
